...
class FireClassifier_Generator(RNGDataFlow):
    ...
    def __iter__(self):
        ...
        for i in indexs:
            image_path, flame, smoke = self.dataset[i]
            ...
            yield [image.astype(np.float32), np.asarray([flame, smoke], dtype = np.float32)]

def generate_dataflow(dataset, option):
    if option['number_of_cores'] == -1:
        option['number_of_cores'] = mp.cpu_count()
    
    ds = FireClassifier_Generator(dataset, option)
