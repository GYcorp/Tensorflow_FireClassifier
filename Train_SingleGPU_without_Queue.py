...
if __name__ == '__main__':
    ...
    output_dic = EfficientNet(train_image_var, True, model_option)
    logits_op, predictions_op = output_dic['logits'], output_dic['predictions']

    class_loss_op = tf.nn.sigmoid_cross_entropy_with_logits(logits = logits_op, labels = train_label_var)
    ...
    # 5. Metrics
    #######################################################################################
    correct_op = tf.equal(tf.greater_equal(predictions_op, 0.5), tf.greater_equal(train_label_var, 0.5))
    ...
    for iter in range(1, flags.max_iteration + 1):
        ...
        _feed_dict = {
            train_image_var : batch_image_data,
            train_label_var : batch_label_data,
            global_step : iter,
        }
